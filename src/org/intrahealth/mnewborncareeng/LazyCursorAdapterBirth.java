...
public class LazyCursorAdapterBirth extends SimpleCursorAdapter  {
	...
	int nameCol,eddCol,slCol,dobCol,mnCol;
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        ...
        if (position == Birth_reg_list.getCurrentSelectedItemId()) {
...
//		dtPicker.init(Integer.parseInt(dt_str[0]),Integer.parseInt(dt_str[1])-1, Integer.parseInt(dt_str[2]), new OnDateChangedListener() {
        ...
        tvEdd.setText("Expected: "+DBAdapter.strtodate(c.getString(eddCol))+" ("+c.getString(mnCol)+")");
        tvSlno.setText(c.getString(slCol));
        if (!c.isNull(dobCol)) 
        	{ 
        	tvDob.setText("DOB: "+DBAdapter.strtodate(c.getString(dobCol)));
    ...
    public void bindView(View v, Context context, Cursor c) {
        ...
        tvSlno.setText(c.getString(slCol));
        if (!c.isNull(dobCol)) { 
        	tvDob.setText("Date of birth: "+DBAdapter.strtodate(c.getString(dobCol)));
        	tvDob.setTextColor(Color.BLUE);
        }
        else tvDob.setText("");
        String imgfile=Environment.getExternalStorageDirectory()+ File.separator + "mcare/pdata/"+
